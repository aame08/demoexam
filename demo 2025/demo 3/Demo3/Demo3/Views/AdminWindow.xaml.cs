    ...
    public partial class AdminWindow : Window
    {
        ...
        private void cbFilter_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (selectedFilter != "Все диапазоны")
            {
                ...
                {
                    case "0-9,99%":
                        currentProducts = currentProducts.Where(p => p.NowDicount >= 0 && p.NowDicount < 10).ToList();
                        break;
                    case "10-14,99%":
                        currentProducts = currentProducts.Where(p => p.NowDicount >= 10 && p.NowDicount < 15).ToList();
                        break;
                    case "15% и более":
                        currentProducts = currentProducts.Where(p => p.NowDicount >= 15).ToList();
                        break;
                }
            }
            else { currentProducts = allProducts; }
